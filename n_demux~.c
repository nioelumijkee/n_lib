...
{
  x->sel = (f<0)?0:(f>x->in_all-1)?x->in_all-1:f;
}
...
void *n_demux_new(t_floatarg f)
{
  ...
  x->in_all = (f<1)?1:(f>MAX_CH)?MAX_CH:f;
  for (i = 0; i < x->in_all; i++)
...
{
  n_demux_class = class_new(gensym("n_demux~"),
			    (t_newmethod)n_demux_new,
			    (t_method)n_demux_free,
			    sizeof(t_n_demux),
			    0, A_DEFFLOAT, 0);
