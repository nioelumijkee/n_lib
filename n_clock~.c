...
#define MINPOS 0
#define MAXPOS 95999 // (96 * 1000) - 1 = 95999
...
void n_clock_outpos(t_n_clock *x)
{
  int i;
  // clip
  x->count = (x->count>MAXPOS)?MAXPOS:x->count;
  // loop
  if (x->loop)
    {
      if (x->count >= x->le)
...
{
  x->ss = (f<MINPOS)?MINPOS:f;
}

void n_clock_se(t_n_clock *x, t_floatarg f)
{
  x->se = (f<MINPOS)?MINPOS:f;
}
...
{
  n_clock_class = class_new(gensym("n_clock~"),
			    (t_newmethod)n_clock_new,
			    0,
			    sizeof(t_n_clock),
			    0, 0, 0);
  ...
  class_addmethod(n_clock_class,(t_method)n_clock_run,   gensym("run"),   A_FLOAT,0);
  ...
  class_addmethod(n_clock_class,(t_method)n_clock_loop,  gensym("loop"),  A_FLOAT,0);
  class_addmethod(n_clock_class,(t_method)n_clock_pos,   gensym("pos"),   A_FLOAT,0);
  class_addmethod(n_clock_class,(t_method)n_clock_ss,    gensym("ss"),    A_FLOAT,0);
  class_addmethod(n_clock_class,(t_method)n_clock_se,    gensym("se"),    A_FLOAT,0);
  class_addmethod(n_clock_class,(t_method)n_clock_ls,    gensym("ls"),    A_FLOAT,0);
  class_addmethod(n_clock_class,(t_method)n_clock_ll,    gensym("ll"),    A_FLOAT,0);
  class_addmethod(n_clock_class,(t_method)n_clock_time,  gensym("time"),  A_FLOAT,0);
}
